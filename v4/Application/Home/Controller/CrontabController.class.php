...
*/
		...
		private function autozhmpBatch($openmj , $autoType=10){
			...
			$where = "ismj=0 and weixin!='' and last_login_time>0";
		...
		*/
 		...
 		private function autonoreply(){
			$msg=M('Message');
			$time=time()-6000;
			$res=$msg->where('fromuid=mj and isread=0 and sendtime>'.$time)->order('id desc')->group('hash')->select();
			foreach($res as $k=>$v){
				if($v['flag']==1){
					unset($res[$k]);
					}
			...
			foreach($res as $k=>$v){
				unset($res[$k]['msgid']);
				$res[$k]['content']=$noreply[array_rand($noreply)];
				$res[$k]['sendtime']=$time;
				$res[$k]['flag']=1;
				}
			$res_rep=$msg->addAll($res);
			if($res_rep){
				echo '二次回复成功';
				}
			}
