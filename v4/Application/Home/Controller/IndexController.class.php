...
class IndexController extends SiteController {
    ...
    public function index($w=""){
		...
		if($querypama['id']){
			...
			$where .= " and ".$querypama['type'].'='.$querypama['id'];			
	...
	public function hot(){
		...
		$list = $User->alias('p')->cache(true,300) ->field('p.thumbfiles,p.title,p.uid,u.avatar,u.user_rank,p.photoid,p.timeline,p.idmd5 as pidmd5,u.idmd5,u.user_nicename,p.hits')->join("__USERS__ as u ON u.id=p.uid")-> where($where) -> order('p.photoid desc') -> limit($Page -> firstRow . ',' . $Page -> listRows) -> select();		
